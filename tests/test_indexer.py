...
def test_save_and_load_index(tmp_path):
    ...
    index_data = {sym.unique_id: sym.to_dict()}
...
def test_index_repo_basic(tmp_path):
    ...
    assert "utils.parse_date" in index
    assert index["utils.parse_date"]["function_name"] == "parse_date"
    assert index["utils.parse_date"]["filepath"] == "utils.py"

    assert "billing.invoice" in index
    assert index["billing.invoice"]["function_name"] == "invoice"
    assert index["billing.invoice"]["filepath"] == "billing.py"
...
def test_index_repo_exclude(tmp_path):
    ...
    assert "keep.keep_func" in index
    assert not any("skip" in k for k in index.keys())
...
def test_index_repo_incremental_cache(tmp_path):
    ...
    assert "a.func_a" in index1
    assert "b.func_b" in index1
    ...
    with patch("blastradius.indexer.parse_file", return_value=[new_sym]) as mock_parse:
        ...
        assert "a.func_a_new" in index3
        assert "b.func_b" in index3  # kept from cache
        mock_parse.assert_called_once_with(str(file_a), str(repo.resolve()))

    # Delete b.py
    file_b.unlink()
    index4 = index_repo(str(repo))
    assert "a.func_a_new" in index4
    assert "b.func_b" not in index4
...
def test_simple_repo_integration(tmp_path):
    ...
    # We expect symbols for all functions in the simple_repo fixture:
    # 1. utils/parser.py: parse_date
    # 2. billing/invoice.py: generate_invoice
    # 3. tests/test_billing.py: test_generate_invoice
    assert "utils.parser.parse_date" in index
    assert index["utils.parser.parse_date"]["function_name"] == "parse_date"

    assert "billing.invoice.generate_invoice" in index

    assert "tests.test_billing.test_generate_invoice" in index

    # Exclude pattern works
    index_ex = index_repo(
        str(dest_dir), exclude=["tests", "__pycache__"], index_dir=str(dest_dir / ".blastradius_ex")
    )
    assert "utils.parser.parse_date" in index_ex
    assert "tests.test_billing.test_generate_invoice" not in index_ex
    ...
    with patch("blastradius.indexer.parse_file", return_value=[mod_sym]) as mock_parse:
        ...
        assert "utils.parser.parse_date_modified" in index_re2
        assert "utils.parser.parse_date" not in index_re2
